...
class MRZViewController: UIViewController {
    ...
    @Published var extractedDocumentNumber: String?
    var onNavigationRequested: (() -> Void)?
    weak var delegate: MRZViewControllerDelegate?
    ...
    private func displayScanningResult(_ result: ParsedResult) {
        ...
        delegate?.didExtractDocumentNumber(extractedDocumentNumber ?? "", isFound)
    ...
    private func addAlertActionAndPresent(_ alertController: UIAlertController) {
        alertController.addAction(.init(title: "OK", style: .cancel, handler: { [ unowned self ] _ in
            showLoadingScreen()
            print("The first one is  \(String(describing: extractedDocumentNumber))")
        }))
    ...
    func showLoadingScreen() {
        ...
        if let loadingVC = loadingVC {
            present(loadingVC, animated: true) {
                self.scanningIsEnabled = false
                self.onNavigationRequested?()
                // Dismiss after 2 seconds
                DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                    // First dismiss the loading screen
                    self.dismiss(animated: true) { [self] in
                        // Then, update the current view in the AppViewModel
                    }
                }
            }
        }
...
//                            DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
//                                self?.appViewModel.currentView = .mrzFace
//                            }
                            // Perform actions based on the extracted document number
                            // For example, present another view or prepare data for API call
                            // You can access the document number using self?.extractedDocumentNumber
                            
                            // Reset isFound if necessary
                            //self?.isFound = false
...
protocol MRZViewControllerDelegate: AnyObject {
